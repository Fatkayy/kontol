...
if(!message.member.hasPermission("MANAGE_ROLES")) return message.channel.reply(`Kamu Mana Bisa Tod!`)        
        //next we define some variables
        const target = message.mentions.members.first() //member = mentions
        if(!target) return message.channel.send('No member!') //when no member is pinged
        const role = message.mentions.roles.first() // roles = mentions
        if(!role) return message.channel.send('No role!') //when no role is specified or pinged
        //now the code!
        await target.roles.add(role) // adding the role to the user
        message.channel.send(` Role Berhasil Ditambahkan ke ${target.metions}`)
    }
}